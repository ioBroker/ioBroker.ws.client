...
class SocketClient {
  ...
  connect(url, options) {
    ...
    try {
      ...
      Object.keys(query).length && (u += `&${Object.keys(query).map((attr) => void 0 === query[attr] ? attr : `${attr}=${query[attr]}`).join("&")}`), this.options?.name && !query.name && (u += `&name=${encodeURIComponent(this.options.name)}`), this.options?.token && (u += `&name=${this.options.token}`), this.socket = new WebSocket(u);
    } catch (error) {
