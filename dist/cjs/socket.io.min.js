...
class SocketClient {
  ...
  log;
  constructor() {
    this.log = { debug: /* @__PURE__ */ __name((text) => console.log(`[${(/* @__PURE__ */ new Date()).toISOString()}] ${text}`), "debug"), warn: /* @__PURE__ */ __name((text) => console.warn(`[${(/* @__PURE__ */ new Date()).toISOString()}] ${text}`), "warn"), error: /* @__PURE__ */ __name((text) => console.error(`[${(/* @__PURE__ */ new Date()).toISOString()}] ${text}`), "error") };
  }
  ...
  connect(url, options) {
    if (this.log.debug("Try to connect"), url && (url = url.split("#")[0]), this.id = 0, this.connectTimer && clearInterval(this.connectTimer), this.connectTimer = null, this.url = this.url || url || window.location.href, this.options = this.options || JSON.parse(JSON.stringify(options || {})), !this.options) throw new Error("No options provided!");
    ...
    try {
      ...
      Object.keys(query).length && (u += `&${Object.keys(query).map((attr) => void 0 === query[attr] ? attr : `${attr}=${query[attr]}`).join("&")}`), this.options?.name && !query.name && (u += `&name=${encodeURIComponent(this.options.name)}`), this.options?.token && (u += `&token=${this.options.token}`), this.socket = new WebSocket(u);
    } catch (error) {
